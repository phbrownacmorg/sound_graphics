...
for i in range(1, num_observers + 1):  
    ...
    for c in conditions:
        ...
        for j in range(num_trials): # 12 experimental trials
            ...
            t['shape'] = shape[combos[j] // (num_trials // 2)]
            ...
            t['point'] = locations[combos[j] % num_locations]
            trials[i][conditions.index(c)].append(t)
...
def run_trial(trial, outfile, observer, condition, w):
    ...
    if trial['shape'] == 'circle':
        ...
        p1 = g.Point(trial['point'][0] - (side/2), trial['point'][1] - (side/2))
        p2 = g.Point(trial['point'][0] + (side/2), trial['point'][1] + (side/2))
