...
def main(args:List[str]) -> int:
    ...
    rect:Rectangle = Rectangle(center, p2, sound=440.0) #sound=pygame.mixer.Sound('sounds/centered_text.wav'))
    rect.setFill('purple')
    rect.draw(w)
    
    p3:Point = Point(575, 450)
    oval:Oval = Oval(center, p3, sound=550.0)  #text='green oval')
    oval.setFill('green')
    oval.draw(w)
    ...
    poly:Polygon = Polygon(p1, center, p3, sound=660.0) #sound='Pointy triangle')
    poly.draw(w)
